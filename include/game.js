...
let _this = module.exports = {
  ...
  winGame: async(peer_id) => {
    ...
    let str = `Выпало число ${_thisGame.game.hash.number} | ${ (_thisGame.game.hash.color ? `чёрное`:`красное`)}!<br><br>`;
    let _name = { 'blue': `красное`, 'black': `чёрное`, 'even': `чётное`, 'noteven': `нечётное` };
  ...
  getKeyboard: () => {
    ...
    return Keyboard.keyboard([
      [
        Keyboard.textButton({ label: 'Красное', payload: { command: 'deposit_blue' }, color: Keyboard.NEGATIVE_COLOR }),
        Keyboard.textButton({ label: 'Банк', payload: { command: 'bank' }, color: Keyboard.POSITIVE_COLOR }),
        Keyboard.textButton({ label: 'Чёрное', payload: { command: 'deposit_black' }, color: Keyboard.PRIMARY_COLOR }),
      ],
      [
      ...
      [
        ...
        Keyboard.textButton({ label: 'Топ игроков', payload: { command: 'top_players' }, color: Keyboard.POSITIVE_COLOR }),
        Keyboard.textButton({ label: 'Вывод', payload: { command: 'output' }, color: Keyboard.PRIMARY_COLOR }),
        ],
      Keyboard.urlButton({ label: 'Пополнить', url: "https://vk.com/coin#t480922940" }),
    ]);
  },
  getPrivateKeyboard: () => {
    var Keyboard = vk.get().Keyboard;
